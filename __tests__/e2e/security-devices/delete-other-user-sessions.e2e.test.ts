...
describe('Terminate all other (exclude current) devices sessions', () => {
  ...
  it('❌ should return 401 if user is not authorized', async () => {
    await request(app)
      ...
      .set('Cookie', 'refreshToken=invalidToken')  // оставляем как есть, для теста 401 норм
      .expect(HttpStatus.Unauthorized)
  });

  it('✅ should delete all other (exclude current) device\'s sessions successfully', async () => {
    const { cookies } = sessions[0];
    const refreshToken = extractCookie(cookies, 'refreshToken');
    
    await request(app)
      .delete(`${SECURITY_DEVICES_PATH}`)
      .set('Cookie', cookies.join('; '))  // ✅ Передаём как строку
      .expect(HttpStatus.NoContent)

    const res = await request(app)
      .get(`${SECURITY_DEVICES_PATH}`)
      .set('Cookie', `refreshToken=${refreshToken}`)  // или cookies.join('; ')
